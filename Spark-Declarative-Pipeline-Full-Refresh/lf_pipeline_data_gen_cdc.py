...
acct_spec = (
    ...
    .withIdOutput()
    .withColumn("acct_id","string",values=[f"00{i}A000000U5DtWAOV" for i in range(10001,10011)])
    .withColumn("acct_name", "string", values=[f"acct_{i}" for i in range(10001,10011)], random=True)
...
accounts_df.write.format("delta").option("mergeSchema", "true").mode("overwrite").saveAsTable("src_accounts")
...
cust_spec = (
    ...
    .withColumn("first_name", "string", values=[f"customer_{i}" for i in range(10001,10011)], random=True)
    .withColumn("last_name", "string", values=[f"custlastname_{i}" for i in range(10001,10011)], random=True)
...
cust_df.write.format("delta").option("mergeSchema", "true").mode("overwrite").saveAsTable("src_customer")
...
# MAGIC %sql
# MAGIC --Validation to make sure all customers have accounts.
# MAGIC select acct_id from src_customer
# MAGIC MINUS
# MAGIC select acct_id from src_accounts;
...
tran_spec = (
    ...
    .withColumn("acct_id","string",values=[f"00{i}A000000U5DtWAOV" for i in range(10001,10011)], random=True)
...
tran_df.write.format("delta").option("mergeSchema", "true").mode("overwrite").saveAsTable("src_acct_transaction")
...
tran_df = tran_spec.build()
tran_df.write.format("delta").option("mergeSchema", "true").mode("append").saveAsTable("src_acct_transaction")

# COMMAND ----------

#Validation to understand number of transactions and distribution against accounts
%sql
with temp_transaction as (
select acct_id, count(1) as cnt from src_acct_transaction group by acct_id)
select min(cnt), max(cnt) from temp_transaction


# COMMAND ----------

# MAGIC %sql
# MAGIC --Validation to identify accounts with no transactions
# MAGIC
# MAGIC select acct_id from src_accounts
# MAGIC MINUS
# MAGIC select acct_id from src_acct_transaction

# COMMAND ----------

# MAGIC %sql
# MAGIC --Validation to identify transactions with no accounts
# MAGIC
# MAGIC
# MAGIC select acct_id from src_acct_transaction
# MAGIC MINUS
